...
const GRUPO_ORION = {
  id: "grupo-orion",
  nome: "ORION",
  dono_id: "user-antonio-orion",
};

// ---------- usuarios ----------
const usuarioAdmin = {
  id: "user-antonio-orion",
  nome_completo: "Antônio",
  email: "antonio.orion@teste.com",
};
...
const todosUsuarios = [usuarioAdmin, ...usuariosPadrao];
...
export async function seedDatabase() {
  ...
  const senhaHashAdmin = await Bun.password.hash(SENHA_ADMIN);
  const senhaHashPadrao = await Bun.password.hash(SENHA_PADRAO);

  const usuarioStatements = [
    {
      sql: `INSERT OR IGNORE INTO usuarios (id, nome_completo, email, senha_hash, criado_em) VALUES (?, ?, ?, ?, ?)`,
      args: [usuarioAdmin.id, usuarioAdmin.nome_completo, usuarioAdmin.email, senhaHashAdmin, agora],
    },
    ...usuariosPadrao.map((u) => ({
      sql: `INSERT OR IGNORE INTO usuarios (id, nome_completo, email, senha_hash, criado_em) VALUES (?, ?, ?, ?, ?)`,
      args: [u.id, u.nome_completo, u.email, senhaHashPadrao, agora],
    })),
  ];

  const grupoStatements = [
    {
      sql: `INSERT OR IGNORE INTO grupos (id, nome, dono_id, criado_em) VALUES (?, ?, ?, ?)`,
      args: [GRUPO_ORION.id, GRUPO_ORION.nome, GRUPO_ORION.dono_id, agora],
    },
  ];

  const grupoMembrosStatements = todosUsuarios.map((u) => ({
    sql: `INSERT OR IGNORE INTO grupo_membros (grupo_id, usuario_id, criado_em) VALUES (?, ?, ?)`,
    args: [GRUPO_ORION.id, u.id, agora],
  }));

  const projetoStatements = projetos.map((p) => ({
    sql: `INSERT OR IGNORE INTO projetos (id, nome, descricao, grupo_id, criado_em) VALUES (?, ?, NULL, ?, ?)`,
    args: [p.id, p.nome, GRUPO_ORION.id, agora],
  }));
  ...
  console.log(
    `Seed concluído: ${todosUsuarios.length} usuários, 1 grupo, ${todosUsuarios.length} membros, ${projetos.length} projetos, ${demandas.length} demandas.`
  );
}
