...
export const demandaController = {
  listar: async (req: Request, res: Response) => {
    ...
    const demandas = await demandaService.listar({
  ...
  buscarPorId: async (req: Request, res: Response) => {
    const demanda = await demandaService.buscarPorId(String(req.params.id));
    res.json(demanda);
  },

  criar: async (req: Request, res: Response) => {
    const criadoPorId = req.usuario?.id ?? "";
    const demanda = await demandaService.criar(req.body, criadoPorId);
    res.status(201).json(demanda);
  },

  atualizar: async (req: Request, res: Response) => {
    const demanda = await demandaService.atualizar(String(req.params.id), req.body);
    res.json(demanda);
  },

  alterarStatus: async (req: Request, res: Response) => {
    const { status } = req.body ?? {};
    const demanda = await demandaService.alterarStatus(String(req.params.id), status);
    res.json(demanda);
  },
