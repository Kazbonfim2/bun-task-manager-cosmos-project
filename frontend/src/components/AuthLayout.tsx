...
import { CheckCircle2, CircleDot, Clock, Snowflake } from "lucide-react";
...
export function AuthLayout({ titulo, subtitulo, children }: AuthLayoutProps) {
  ...
  return (
    <main className="flex-1 w-full h-[calc(100dvh-3.5rem)] flex flex-col lg:grid lg:grid-cols-[40%_60%] xl:grid-cols-[38%_62%] relative overflow-hidden bg-background">
      {/* Painel Esquerdo (Apenas Desktop) */}
      <aside className="relative hidden lg:flex flex-col justify-between overflow-hidden border-r border-border/40 bg-gradient-to-br from-muted/50 via-muted/20 to-background p-6 xl:p-8 text-foreground h-full">
        ...
        <div className="relative z-10 space-y-2">
          <h2 className="font-heading min-h-12 text-xl xl:text-2xl font-bold tracking-tight text-foreground leading-snug">
            {textoDigitado}
            <span
              className="ml-1 inline-block h-5 w-1.5 translate-y-0.5 bg-primary align-baseline animate-pulse rounded-xs"
              aria-hidden="true"
            />
          </h2>
        ...
        {/* Mockups de Cards Flutuantes Compactos */}
        <div className="relative z-10 my-auto flex flex-col items-center justify-center py-2 gap-0 scale-90 xl:scale-95 origin-center">
          {/* Card 1 - Topo */}
      ...
      <section className="flex flex-1 flex-col items-center justify-center p-4 sm:p-6 lg:p-8 w-full h-full overflow-y-auto lg:overflow-hidden">
        <div className="w-full max-w-sm sm:max-w-md lg:max-w-lg my-auto space-y-3 sm:space-y-4">
          {/* Header */}
          <div className="flex flex-col items-center text-center gap-1 mb-1">
            <div className="flex lg:hidden items-center justify-center gap-2 mb-0.5">
              <div className="size-7 rounded-lg bg-primary/10 flex items-center justify-center text-primary border border-primary/20 shadow-xs">
                <Snowflake className="size-4 animate-sway" aria-hidden="true" />
              </div>
              <span className="font-birthstone text-2xl font-normal tracking-wide text-foreground leading-none">
                Snow/Board*
              </span>
            </div>
